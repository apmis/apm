...
const NAMES = ['Rashidi Adebayo', 'Modupe Alabi', 'Ifeanyi Okafor', 'Grace Okoro', 'Sikiru Lawal',
  ...
  'Aisha Bello', 'Segun Akinlade', 'Titilayo Ogun', 'Emeka Nwosu', 'Bisi Ademola'];
...
async function seed() {
  ...
  const lgaRecords = await db.collection('lgas').find().sort({ displayOrder: 1 }).toArray();
  ...
  for (const lga of lgaRecords) {
    for (let i = 1; i <= 11; i++) {
      wardDocs.push({
        ...
        lgaId: lga._id.toString(), displayOrder: wardOrder++, ...base,
      });
    }
  }
  ...
  for (const ward of wardRecords.slice(0, 33)) {
    ...
    for (let i = 1; i <= 5; i++) {
      puDocs.push({
        code: `${ward.code}PU${String(i).padStart(3, '0')}`, name: `${ward.name} Polling Unit ${i}`,
        wardId: ward._id.toString(), lgaId: ward.lgaId,
        registeredVoters: Math.floor(Math.random() * 500) + 200,
        location: { type: 'Point', coordinates: [3.9 + Math.random(), 7.4 + Math.random()] },
        ...base,
      });
    }
  }
  ...
  // --- Stakeholders ---
  const stakeDocs: any[] = [];
  const roles = ['traditional', 'religious', 'political', 'community', 'youth', 'women', 'professional', 'market'];
  ...
  await db.collection('incidents').insertMany(incidentDocs.map((d) => ({
    ...d, geography: { pollingUnitId: firstPu._id.toString(), lgaId: firstPu.lgaId },
    reportedBy: 'seed-system', reportedAt: new Date(), ...base,
  })));
  ...
  const resultDocs = puRecords.slice(0, 15).map((pu) => {
    ...
    return {
      ...
      validation: { isMathematicallyValid: true, warnings: [], checksum: null, duplicateScore: 0 },
  ...
  await db.collection('notifications').insertMany(gotvDocs.map((d) => ({
