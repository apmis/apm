...
export async function sendSms(to: string, message: string): Promise<TermiiSendSmsResponse> {
  const apiKey = process.env.TERMII_API_KEY || '';
  const senderId = (process.env.TERMII_SENDER_ID || 'APM Campaign').trim();
  const baseUrl = process.env.TERMII_BASE_URL || 'https://v3.api.termii.com/api';
